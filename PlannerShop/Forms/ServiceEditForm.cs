...
namespace PlannerShop.Forms
{
    public partial class ServiceEditForm : Form
    {
        ...
        public ServiceEditForm(string idProdotto)
        {
            InitializeComponent();
            this.idServizio = idProdotto;
        ...
        {
            DataTable dt = ModelServizi.getServizioById(idServizio);
            if (dt.Rows.Count != 0)
            {
                dtpData.Text = dt.Rows[0]["DATA"].ToString();
                txtMarca.Text = dt.Rows[0]["NOME"].ToString();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dltFrm.result)
                {
                    ModelProdotti.deleteProdotto(idServizio);
                    isDelete = true;
                    this.Close();
                }
            }
